...
-- Create missing function count_devs_with_more_achievements
CREATE OR REPLACE FUNCTION count_devs_with_more_achievements(target_count bigint)
RETURNS bigint AS $$
DECLARE
  devs_count bigint;
BEGIN
  SELECT count(*) INTO devs_count
  FROM (
    SELECT developer_id
...
-- Create missing function top_achievers
CREATE OR REPLACE FUNCTION top_achievers(lim integer)
RETURNS TABLE(developer_id bigint, ach_count bigint) AS $$
BEGIN
  RETURN QUERY
  SELECT da.developer_id, count(*)::bigint AS ach_count
  FROM public.developer_achievements da
  GROUP BY da.developer_id
