...
// GET /api/dev/[username]/connection - Check if following target user
export async function GET(
  ...
  try {
    ...
    if (error && error !== "GitHub token missing") {
      // Unauthenticated visitor - they aren't following anyone
      return NextResponse.json({ isFollowing: false });
    }

    // If authenticated but GitHub token is missing from the DB, check publicly
    if (!token && dev) {
...
// POST /api/dev/[username]/connection - Follow target user
...
// DELETE /api/dev/[username]/connection - Unfollow target user
